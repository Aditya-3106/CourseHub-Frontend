...
import { PlayCircle, Lock, CheckCircle2, Loader2, Video, FileText, ArrowLeft } from 'lucide-react';
...
export default function CourseLanding() {
  ...
  const [data, setData] = useState(null);      // { course, access }
  ...
  useEffect(() => {
    const fetchCourseDetails = async () => {
      try {
        // FIXED: Correct backend route is GET /course/:id/content
        const res = await api.get(`/course/${id}/content`);
        // Backend returns: { success: true, data: { course, access } }
  ...
  const handlePurchase = async () => {
    ...
    setPurchasing(true);
    try {
      // FIXED: Correct backend route is POST /course/purchase/:id
      await api.post(`/course/purchase/${id}`);
      toast.success('Course purchased successfully! Happy learning 🎉');
      navigate(`/play/${id}`);
    } catch (err) {
      const msg = err.response?.data?.message || 'Failed to purchase course';
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-16 bg-zinc-950 flex flex-col items-center">
        <div className="w-full bg-zinc-900 border-b border-zinc-800 animate-pulse h-80" />
    ...
    return (
      <div className="min-h-screen pt-24 bg-zinc-950 flex items-center justify-center">
        <div className="text-center text-zinc-400">
          ...
          <Link to="/" className="text-indigo-400 hover:text-indigo-300 flex items-center gap-2 justify-center">
  ...
  const hasAccess = access?.fullAccess;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 relative z-20">
          <div className="max-w-3xl">
            ...
            <p className="text-lg text-zinc-400 mb-8 leading-relaxed">
              {course.description || 'Take your development skills to the next level with this comprehensive, project-based course.'}
            </p>
            ...
            <div className="space-y-4">
              {course.sections?.length > 0 ? (
                course.sections.map((section, idx) => (
                  <div key={section.id} className="glass-card border border-zinc-800/60 overflow-hidden">
            ...
            <div className="sticky top-24 glass-card p-6 border-zinc-800/80 shadow-2xl">
              ...
              {hasAccess ? (
                <button
                  ...
                  className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-lg flex items-center justify-center gap-2 transition-all mb-4"
                >
